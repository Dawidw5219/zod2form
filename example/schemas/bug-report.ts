...
export const schema = f.object({
  title:      f.string().min(5).max(120).trim().field("text").label("Title").placeholder("Crash when clicking Save"),
  ...
  pageUrl:    f.string().url().field("text").label("Page URL where bug occurs").placeholder("https://app.example.com/dashboard"),
  ...
  steps:      f.string().min(20).field("textarea").label("Steps to reproduce").placeholder("1. Go to...\n2. Click on...\n3. See error").props({ rows: 4 }),
